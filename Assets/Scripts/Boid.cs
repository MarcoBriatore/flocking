...
public class Boid : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        this.height = this.GetComponentInChildren<SpriteRenderer>().sprite.rect.size.y;
        this.width = this.GetComponentInChildren<SpriteRenderer>().sprite.rect.size.x;
    }
    ...
    // this magic number is a aproximation of the height of the asset 
    private void WrapAround()
    {
        if (Position.x < -(Screen.width / this.width)) Position = new Vector2(-Position.x, Position.y);
        else if (Position.y < -(Screen.height / this.width)) Position = new Vector2(Position.x, -Position.y);
        else if (Position.x > Screen.width / this.width) Position = new Vector2(-Position.x, Position.y );
        else if (Position.y > Screen.height / this.width) Position = new Vector2(Position.x, -Position.y);
    }
}